# {{firma}}: die Seite ans Netz bringen
...
- Die Datei `{{kurzname}}-website.zip`
- Einen Hosting-Zugang mit PHP, auf den eigenen Namen
- Eine eigene E-Mail-Adresse zum Testen des Formulars
- Das Passwort für den Pflegebereich (muendlich erhalten, steht in keiner Datei)
...
- [ ] **5. Die Dateien hochladen.** Im Kundenbereich den Dateimanager
  ...
  `public_html`.
  Die Datei `{{kurzname}}-website.zip` dort hochladen und im Dateimanager
...
- [ ] **8. Den Empfänger des Kontaktformulars umstellen.** Wichtig, sonst
  ...
  `{{mail}}`. Diese Adresse durch die eigene ersetzen, speichern.
...
- [ ] **14.** Die Adresse mit `/pflege` dahinter aufrufen und anmelden.
  Das Auslieferpasswort haben Sie muendlich bekommen; es steht in keiner Datei. Es erscheint die Übersicht
  mit den Reitern.
...
- [ ] **24. Termin mit {{inhaber}}.** Freigabeblatt mitnehmen und
...
- [ ] **27. Dateien hochladen und die Punkte 11 bis 23 nochmal prüfen.**
  ...
  `{{mail}}` steht und nicht die Testadresse aus Schritt 8.
