...
Sein bisheriger Auftritt liegt unter `{{domain}}`.
...
`kunde1234.hoster.de`. Dort läuft die Seite schon, bevor `{{domain}}`
...
10. **Kontaktformular ausfüllen und absenden. Kommt die Mail an?**
    Empfänger steht in `pflege/formular.php`: `{{mail}}`. Dieser
...
5. **Sofort prüfen**, nicht später:
   - Seite unter `{{domain}}` aufrufen.
   - Eine Testmail **an** `{{mail}}` schicken. Kommt sie an?
...
2. **Passwort ändern lassen.** Er tippt es selbst ein, wir sehen es nicht.
   Das ausgelieferte Passwort gilt danach nicht mehr.
3. **Zugangsdaten übergeben:** Hosting-Zugang, Domain-Zugang, die Adresse
   `{{domain}}/pflege`. Alles gehört ihm, nicht uns.
